...
var TabManager = React.createFactory(React.createClass({
	...
	render: function render() {var _this = this;
		...
		React.DOM.div({ className: "hrCont " + (!haveSess ? "hidden" : "") },
		React.DOM.div({ className: "hrDiv" },
		React.DOM.span({ className: "hrSpan" },
		"Saved windows"))),

		this.state.sessionsFeature ? this.state.sessions.map(function (window) {
			return Session({
				window: window,
				tabs: window.tabs,
				incognito: window.incognito,
		...
		React.DOM.table({},
		React.DOM.tr({},
		React.DOM.td({ className: "one" }, React.DOM.input({ className: "searchBoxInput", type: "text", placeholder: "Search tabs...", tabIndex: "1", onChange: this.search, ref: "searchbox" })),
		React.DOM.td({ className: "two" },
	...
	componentDidMount: function componentDidMount() {
		...
		browser.tabs.onAttached.addListener(this.update.bind(this));
		browser.tabs.onActivated.addListener(this.update.bind(this));
		browser.windows.onFocusChanged.addListener(this.update.bind(this));
		browser.windows.onCreated.addListener(this.update.bind(this));
		browser.windows.onRemoved.addListener(this.update.bind(this));

		browser.storage.onChanged.addListener(this.sessionSync.bind(this));
	...
	sessionSync: async function sessionSync() {
	...
	deleteTabs: async function deleteTabs() {var _this2 = this;
	...
	addWindow: async function addWindow() {var _this3 = this;
	...
	pinTabs: async function pinTabs() {var _this4 = this;
							...
							if(_w.state != "minimized") {
								...
								for(var _t of _w.tabs) {
									...
									if(!selectedTab) {
										...
										console.log("select nexttt window ", _t.id);
										this.selectWindowTab(_w.id, tabPosition);
										found = true;
										break;
									}
									
									console.log(_t, _w.id == selectedTab);
								}
								prev = _w.id;
							}
						}
							...
							if(_w.state == "minimized") {
								...
								for(var _t of _w.tabs) {
									...
									if(!selectedTab) {
										...
										found = true;
										break;
									}else if(selectedTab == _w.id) {
										console.log("select next window ", selectedNext);
										if(goDown) {
											selectedNext = true;
										}else if(prev){
											console.log("select prev window ", prev);
											this.state.selection = {};
											this.select(prev);
											found = true;
											break;
	...
	selectWindowTab: function selectWindowTab(windowId, tabPosition) {
		for(var _w of this.state.windows) {
	...
	scrollToTab: function scrollToTab(id) {
		var tabs = document.getElementsByClassName('tab-' + id);
		if (!!tabs && tabs.length > 0) {
			if(!this.elVisible(tabs[0])) {
				tabs[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
			}
		}
	},
	...
	selectTo: function selectTo(id, tabs) {
		console.log(id, tabs);
		var activate = false;
		...
		this.scrollToTab(this.state.lastSelect);
	...
	drop: async function drop(id, before) {
		var _this5 = this;
		var tab = this.state.tabsbyid[id];
	...
	dropWindow: async function dropWindow(windowId) {var _this6 = this;
	...
	toggleBadge: async function toggleBadge() {
	...
	exportSessionsText: function sessionsText() {
		this.setState({
			bottomText: "Allows you to export your saved windows to an external backup" });

	},
	importSessions: function importSessions(evt) {
		try {
	...
	importSessionsText: function sessionsText() {
		this.setState({
			bottomText: "Allows you to restore your saved windows from an external backup" });

	},
	toggleHide: async function toggleHide() {
