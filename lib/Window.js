"use strict";var Window = React.createFactory(React.createClass({
	...
	render: function render() {var _this = this;
		...
		if (!hideWindow) {
			...
			if (name) {
				tabs.unshift(
				React.DOM.h3({ className: "center" }, name));

			} else {
				var hostnames = [];
				for (var i = 0; i < tabs.length; i++) {
					if (!!tabs[i].props && !!tabs[i].props.tab && !!tabs[i].props.tab.url) {
						var hostname = new URL(tabs[i].props.tab.url).hostname;
				...
				if (hostnames.length > 0) {
					tabs.unshift(
					React.DOM.h3({ className: "center" }, this.topEntries(hostnames).join("")));
	...
	topEntries: function topEntries(array)
	{
		...
		var more = 0;
		while (newTypesArray.length > 2) {
			newTypesArray.pop();
			more++;
		}
		for (var i = 0; i < newTypesArray.length; i++) {
			if (i > 0) {
				newTypesArray[i] = ", " + newTypesArray[i];
			}
		};
		if (more > 0) {
			newTypesArray.push(" & " + more + " more");
		}
