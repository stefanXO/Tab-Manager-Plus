"use strict";var Tab = React.createFactory(React.createClass({
	...
	render: function render() {
		...
		var tabDom = {
			...
			this.props.searchActive ? "search-active " : "") + " tab-" + this.props.tab.id + " " + (
	...
	click: function click(e) {
		...
		if (e.button === 1) {
			...
			browser.tabs.update(this.props.tab.id, { active: true }).then(function() {
				browser.windows.update(this.props.window.id, { focused: true }).then(function() {
