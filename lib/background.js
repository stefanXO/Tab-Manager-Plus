...
onMessage("update_tab_count", updateTabCount);
...
async function tabAdded(tab) {
	...
	try {
		var tabLimit = await getItem("tabLimit");
	} catch (e) {
		var tabLimit = 0;
	}
	if (tabLimit > 0) {
...
async function openPopup() {
	...
	try {
		openInOwnTab = await getItem("openInOwnTab");
	} catch (e) {
		openInOwnTab = false;
	}
	if (openInOwnTab) {
...
async function windowActive(windowId) {
	...
	var windows = await getItem("windowAge");
	...
	await setItem("windowAge", windows);
...
onMessage("reload_popup_controls", setupPopup);

(async function () {
	var windows = await browser.windows.getAll({ populate: true });
	await setItem("windowAge", []);
