...
function createWindowWithTabs(tabs, isIncognito) {
	...
	browser.windows.create({ tabId: first.id, incognito: !!isIncognito, focused: true }, function (first, t, w) {
		browser.tabs.update(first.id, { pinned: first.pinned });
		if (t.length > 0) {
...
function updateTabCount() {
	...
	if (run) {
		browser.tabs.query({}, function (result) {
			...
			browser.browserAction.setBadgeBackgroundColor({ color: "purple" });
			var toRemove = [];
				...
				browser.tabs.query({}, function(tabs){
					for (var i = 0; i < tabs.length; i++) {
						...
						if(tab.url.indexOf("popup.html") > -1) {
							browser.tabs.highlight({windowId: tab.windowId, tabs: tab.index}, function() {});
							return;
						}
					}
	...
	if (!windowId || windowId < 0) {
		...
		function (windowId, result) {
			if (result) {
				...
				browser.system.display.getInfo(function (windowId, displaylayouts) {
					...
					browser.windows.getAll({ populate: true }, function (windowId, windows) {
						var monitor = -1;
						...
						for (var i = windows.length - 1; i >= 0; i--) {
							if (windows[i].id != windowId) {
								if (is_in_bounds(windows[i], window.displayInfo[monitor])) {
									browser.windows.update(windows[i].id, {
										"state": "minimized" });
								}
							}
						};
